...
export default function LendSection() {
  ...
  const [userNXFBalance, setUserNXFBalance] = useState()
  ...
  useEffect(()=>{
   const getTheData = async() => {
     ...
     try {
       ...
       setuserXFIBalance(parseFloat(parseXFIbalance.toString()).toFixed(5))
       const NXFBalance = await tokenContractReadSettings.balanceOf(address)
       const parseNXFBalance = parseFloat(formatUnits(NXFBalance, 18)).toFixed(5);
       setUserNXFBalance(parseNXFBalance.toString())
   ...
   getTheData();  
  }, [userXFIBalance, userNXFBalance, userUSDTBalance, userDAIBalance, loading])
  ...
  const showMyCreatedLoans = () => {
    ...
    return (
        ...
        <div className="text-[#ccc] text-[90%]">Lend on Revolend to enjoy huge rewards (P2P)</div>
 ...
 {searchData.length > 0 ? 
  (<div className="my-[1cm] bg-[#000] p-[0.5cm] rounded-xl" style={{boxShadow:"2px 2px 2px 2px #222"}}>
    ...
    <div className="m-[0.2cm]" style={{display:"inline-block"}}><span className="bg-[#502] rounded-md px-[0.3cm] py-[0.1cm] text-[80%]" style={{border:"2px solid #333"}}>Token to borrow</span> XFI</div>
    <div className="m-[0.2cm]" style={{display:"inline-block"}}><span className="bg-[#502] rounded-md px-[0.3cm] py-[0.1cm] text-[80%]" style={{border:"2px solid #333"}}>Amount to borrow</span> {parseFloat(data.amount.toString() * 10**-18).toFixed(10)} XFI</div>
    ...
    <div className="m-[0.2cm]" style={{display:"inline-block"}}><span className="bg-[#502] rounded-md px-[0.3cm] py-[0.1cm] text-[80%]" style={{border:"2px solid #333"}}>Repayment amount</span> {parseFloat(data.repaymentAmount.toString() * 10**-18).toFixed(10)} XFI</div>
    ...
    {(data.collateral.toString().toLowerCase()) === tokenContractAddress.toLowerCase() && (<span> NXF</span>)} 
