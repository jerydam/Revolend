...
import {ITreasury, IStakERC20, IRevoNft, ISwapper, IP2PLending} from "./interfaces.sol";
...
modifier validateAction(uint _proposalId) {
    ...
    modifier hasMinimumRevoBalance(address _member) {
        require(IERC721(RevoNft).balanceOf(_member) > 0, "not qualified");
        require(
            IERC20(RevoErc20).balanceOf(_member) >= (100 ether),
            "not qualified"
        );
        _;
    }
   ...
   function usernameExists(string memory _username) public view returns (bool) {
    ...
    ) external hasMinimumRevoBalance(_member) {
    ...
    function mintNft(string memory _uri) public {
        uint stakedbalance = IStakERC20(stakeErc20)
            .stakeassets(msg.sender, RevoErc20)
            .capital;
        require(stakedbalance >= 1000 ether, "not qualified");
        IRevoNft(RevoNft).safeMint(msg.sender, _uri);
    }
