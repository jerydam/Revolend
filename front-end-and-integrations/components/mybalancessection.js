...
import {
    ...
    export default function MyBalancesSection({displayComponent, setDisplayComponent, changeBg3, changeBg4, changeBg5}) {
              ...
              // lets read data for the Metrics section using inbuilt functions and abi related read functions
               const [RVLBalance, setRVLBalance] = useState()
    ...
    return (
        ...
        <div className="text-[#ccc] text-[90%]">Manage all your assets on RevoLend</div>
        <div className="text-center mt-[0.4cm]">
            <div className="text-center m-[0.4cm]" style={{display:"inline-block"}}>
                <div className="font-[500] text-[110%]">RVL Balance</div>
                {RVLBalance > 0 ? (<div className="text-[#aaa]">{Intl.NumberFormat().format(parseFloat(RVLBalance).toFixed(10))} RVL</div>) : (<span>0</span>)}
            </div>
            <div className="text-center m-[0.4cm]" style={{display:"inline-block"}}>
                <div className="font-[500] text-[110%]">RVL Price</div>
                {tokenPrice ? (<div className="text-[#aaa]">≈ ${tokenPrice}</div>) : (<span>0</span>)}
            </div>
            <div className="text-center m-[0.4cm]" style={{display:"inline-block"}}>
                <div className="font-[500] text-[110%]">ETH Balance</div>
                {userETHBalance > 0 ? (<div className="text-[#aaa]">{parseFloat(userETHBalance).toFixed(10)} ETH</div>) : (<span>0</span>)}
            </div>
            <div className="text-center m-[0.4cm]" style={{display:"inline-block"}}>
        ...
        <div className="grid lg:grid-cols-2 grid-cols-1 gap-4 mt-[1cm]">
            <div className="grid-cols-1 bg-[#000] p-[0.5cm] rounded-xl" style={{boxShadow:"2px 2px 2px 2px #333"}}>
                <div className="font-[500] text-[#fff] bg-[#209] px-[0.4cm] py-[0.1cm] rounded-md mb-[0.2cm]" style={{display:"inline-block"}}>$RVL</div>
               <div className="text-[#ccc] font-[500] underline">What is RVL?</div>
               <div className="text-[#aaa] text-[90%]">
               RVL serves as the core utility token within the RevoLend ecosystem. It plays a crucial role in the lending process, where users can use RVL as collateral when creating or securing loans on the platform. Beyond its functional role in lending, RVL is also positioned as a key incentive mechanism for the community. In line with our commitment to user engagement and platform growth, RVL will be distributed in future reward programs, including potential airdrops, to recognize and incentivize active community participation. Alternatively, you can purchase $RVL using the swap feature. As the ecosystem evolves, RVL will continue to be at the center of RevoLend's utility, governance, and reward structure.
               </div>
               <button onClick={(e) => setDisplayComponent("swaptokens") & changeBg5(e)} className="text-center px-[0.4cm] py-[0.2cm] bg-[#209] w-[100%] mt-[0.3cm] generalbutton text-[#fff] rounded-md">Buy RVL</button>
            </div>
            <div className="grid-cols-1 bg-[#000] p-[0.5cm] rounded-xl" style={{boxShadow:"2px 2px 2px 2px #333"}}>
                ...
                <div className="text-[#aaa] text-[90%]">
                On RevoLend, users can engage in P2P lending/borrowing using supported tokens through a decentralized and transparent system.
