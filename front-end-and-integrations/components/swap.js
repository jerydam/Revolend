...
export default function SwapSection() { 
           ...
           useEffect(()=>{
            const getTheData = async() => {
              ...
              try {
                ...
                console.log(parseRVLbalance.toString())
          ...
          setLoading(true)
           ...
           try {
            if (tokenA === "1" & tokenB !== "1"){
              ...
              const approveSwapToSpendRVL = await tokenContractWriteSettings.approve(swapContractAddress, parseUnits(tokenAamount, 18))
            }
            else if (tokenA === tokenB){
              setSameTokenError(true)
              setTimeout(() => {
    ...
    return (
        ...
        <div className="text-[#ccc] text-[90%]">Swap your favorite tokens effortlessly using RevoLend's Swap feature, powered by ChainLink oracles for accurate price calculations</div>
        ...
        <div className="swapdiv bg-[#111] p-[0.5cm] rounded-xl " style={{boxShadow:"2px 2px 2px 2px #333"}}>
         ...
         <span className='text-[#000] float-left'>Bal: ≈ {tokenA == "0" && (<span>{userETHBalance}</span>)} {tokenA == "1" && (<span>{userUSDTBalance}</span>)} {tokenA == "2" && (<span>{userDAIBalance}</span>)} {tokenA == "3" && (<span>{userRVLBalance}</span>)}</span>
        ...
        <div className="swapdiv bg-[#111] p-[0.5cm] rounded-xl" style={{boxShadow:"2px 2px 2px 2px #333"}}>
         ...
         <div className='mt-[1.5cm] clear-both font-[500]'>
         <span className='text-[#000] float-left'>Bal: ≈ {tokenB == "0" && (<span>{userETHBalance}</span>)} {tokenB == "1" && (<span>{userUSDTBalance}</span>)} {tokenB == "2" && (<span>{userDAIBalance}</span>)} {tokenB == "3" && (<span>{userRVLBalance}</span>)}</span>
         </div>
         </div>
        </div>
